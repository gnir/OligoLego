function AppMSBS(MSPath,BSPath,StreetsPath,PTablePath,MaxAvoid,SavePath,varargin)
%% AppendingStreetsMSBS
...
end % for i=1:size(MainCell,1) % find number of distinguished indices

for i=1:size(BackCell,1) % find number of distinguished indices
    MToes(end+1)=(BackCell{i,11});
    BToes(end+1)=(BackCell{i,10});
end % for i=1:size(BackCell,1) % find number of distinguished indices
        ...
        if IFlag==1 && rflag==1
            ...
            for k=StartToe+1:size(PairsMat,1)
                if PairsMat(PrF(end),k)<=3 && flag==0
                    OligoPaints{i,:}=strcat(ReverseComplement(Streets(PrF(end),:),1),MainCell{i,8},ReverseComplement(Streets(k,:),1));
                    OPCords(i,1)=str2double(MainCell{i,6});
                    OPCords(i,2)=str2double(MainCell{i,7});
                    PrR(end+1)=k;
                    q=size(PrR,2)+1;
        ...
        elseif IFlag==1 && rflag==0 && fflag==1
            for j=StartToe+1:size(PairsMat,1)
                if PairsMat(PrF(end),j)<=3 && flag==0
                    OligoPaints{i,:}=strcat(ReverseComplement(Streets(PrF(end),:),1),MainCell{i,8},ReverseComplement(Streets(j,:),1));
                    OPCords(i,1)=str2double(MainCell{i,6});
                    OPCords(i,2)=str2double(MainCell{i,7});
                    PrR(end+1)=j;
                    q=size(PrR,2)+1;
        ...
        elseif MainCell{i,10}==MainCell{i+1,10} && MainCell{i,11}~=MainCell{i+1,11}
            ...
            for t=i:-1:1
                if MainCell{i+1,11}==MainCell{t,11} && rflag==0
                    for l=1:StartToe
                        if strcmp(OligoPaints{t,1}(end-StreetsLength+1:end),ReverseComplement(Streets(l,:),1))
                            IFlag=1; rflag=1;
                            PrR(end+1)=l;
                            q=size(PrR,2)+1;
                            break
                        end % if strcmp(OligoPaints{t,1}(end-StreetsLength+1:end),ReverseComplement(Streets(l,:),1))
        ...
        if isempty(BackCell(BackCell{i,11}==mainUnique))
            if BFlag==1 && MFlag==1
                ...
                if flag==1
                    flag2=0;
                    for j=StartToe+1:size(PairsMat,1)
                        if PairsMat(PrR(end),k)<=3 && flag2==0
                            PrF(end+1)=j;
                            flag2=1;
                            break
                ...
                OligoPaints{end+1,:}=strcat(ReverseComplement(Streets(PrF(end),:),1),BackCell{i,8},ReverseComplement(Streets(PrR(end),:),1));
...
if exist('UniF','var')
    ...
    UniversalMS=Streets(PrF(end),:);
    U_MS_N=PrF;
    UniversalBS=Streets(PrR(end),:);
    U_BS_N=PrR;
end % if exists('UniF','var')
%% Find Streets chosen
k=1; s=1; t=1; l=1; 
clear OligoMain OligoSeqMain ToeMain OligoBack
...
for i=1:size(ToeChosen,2)-1
    for j=i+1:size(ToeChosen,2)
        if sum(Streets(ToeChosen(i),:)==Streets(ToeChosen(j),:))>StreetsLength-2
            ToeSame(k,:)=Streets(i,:);
            idx(k,1)=ToeChosen(i); idx(k,2)=ToeChosen(j);
            k=k+1;
        end % if sum(Streets(ToeChosen(i),:)==Streets(ToeChosen(j),:))>StreetsLength-2
    end % for j=i+1:size(ToeChosen,2)
end % for i=1:size(ToeChosen,2)-1
if k>1
    errordlg(['You have ',num2str(k),' streets with 2 or less nt difference'])
    prefix='Error Streets alike.txt';
    outputError=strcat(SavePath,prefix);
    fileID=fopen(outputError,'w');
    fprintf(fileID,'You have streets with 2 or less nt difference\n');
    fclose(fileID);
    prefix='Streets alike.txt';
    outputError=strcat(SavePath,prefix);
    fileID=fopen(outputError,'w');
    fprintf(fileID,'%6.0f',idx(:,1));
    fclose(fileID);
end % if k>1
        ...
        for k=1:size(MainCell,1)
            if str2double(MainCell{k,6})>=BcountsID(1,i) && str2double(MainCell{k,7})<=BcountsID(2,i)
                for j=1:size(OligoPaints,1) 
                    if strcmp(OligoPaints{j,1}(2*StreetsLength+1:end-2*StreetsLength),MainCell{k,8}) && flag==0
        ...
        for t=1:size(OPLength,2)
            ...
            if LenDiff>0 && OPLength(i,t)~=0 && ~isnan(OPLength(i,t))
                ...
                for k=1:2:LenDiff
                    ...
                    if k+1<=LenDiff
                        OligoPaints{extendIdx(i,t),1}=strcat(OligoPaints{extendIdx(i,t),1}(1:end-StreetsLength),'T',OligoPaints{extendIdx(i,t),1}(end-(StreetsLength-1):end));
                    else
                        break
                    end % if k+1<=LenDiff
...
h=waitbar(0,'Counting probes per BackStreet');
for i=1:size(ToeBack,2)
    for j=1:size(OligoPaints,1)
        if strcmp(OligoPaints{j,1}(end-(2*StreetsLength-1):end-StreetsLength),ReverseComplement(Streets(ToeBack(i),:),1))
...
prefix='BS_IDs.txt'; % print backstreets streets number, BS ID, toe seq, bridge seq and Rev primer seq.
outputBIDs=strcat(SavePath,prefix);
fileID=fopen(outputBIDs,'w');
fprintf(fileID,'BSRegionID\tID\tStreetNum\tStreetSequence\tBridgeSequence\tReversePrimerSeq\n');
for i=1:size(ToeBack,2)
    Bridge=Streets(ToeBack(i),:);
    RPrimer=Streets(ToeBack(i),:);
    fprintf(fileID,'%s\t%d\t%d\t%s\t%s\t%s\n',uniqueBIDs{i,1},BToes(i),ToeBack(i),Streets(ToeBack(i),:),Bridge,RPrimer);
end
fclose(fileID);
